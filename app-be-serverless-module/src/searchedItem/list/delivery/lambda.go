...
import (
	...
	searchedItemList "diskon-hunter/price-monitoring/src/searchedItem/list/function"
)
...
func LambdaHandlerV1(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	var res searchedItemList.QueryV1DataResponse
	if errObj == nil {
		cmd := searchedItemList.QueryV1{
			Version:         "1",
			RequesterUserId: jwttoken.GetUserId(token),
		}

		res, errObj = searchedItemList.QueryV1Handler(
			ctx,
			searchedItemList.QueryV1Dependencies{
				Logger:         logger,
