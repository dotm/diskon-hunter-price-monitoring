...
resource "aws_dynamodb_table" "monitored_link" {
  lifecycle {
    prevent_destroy = true
  }

  name         = "${var.deployment_environment_name}-${var.project_name_short}-StlMonitoredLinkDetail"
  ...
  hash_key = "HubMonitoredLinkUrl" #attribute used as partition key (beware of hot partition problem)
  ttl {
  ...
  attribute {
    name = "HubMonitoredLinkUrl"
    type = "S" #(S)tring, (N)umber or (B)inary
  }
}

resource "aws_dynamodb_table" "user_monitors_link" {
  lifecycle {
    prevent_destroy = true
  }
  ...
  name         = "${var.deployment_environment_name}-${var.project_name_short}-StlUserMonitorsLinkDetail"
  billing_mode = "PAY_PER_REQUEST" #Default value is PROVISIONED. On demand is PAY_PER_REQUEST.

  #if no range_key, pk value must be unique, else pk-sk combination value must be unique
  hash_key  = "HubUserId" #attribute used as partition key (beware of hot partition problem)
  range_key = "HubMonitoredLinkUrl"           #attribute used as sort key
  global_secondary_index {
    name            = "LinkMonitoredByUserGSI"
    hash_key        = "HubMonitoredLinkUrl"
    range_key       = "HubUserId"
    projection_type = "ALL"
  }
  ttl {
